...
**技能位于 DSH 用户技能根 `~/.agents/skills/<name>/SKILL.md`，用 DSH 的 skill 工具按名加载**，每个 skill 为一个子目录，入口文件为 `SKILL.md`。

Agent 在执行流程中引用某个 skill 时，必须：
1. 用 DSH 的 skill 工具加载 {skill_name} 的完整定义
...
- **RC.1**: 读取 `~/.agents/skills/driver-code-reviewer/SKILL.md`，按其「触发格式」校验用户输入（本地路径）。严禁回退到旧的 6 维 checklist 流程。**✅ RC.1 完成**
...
**状态初始化**（仅新工作流）: `bash ~/.agents/skills/nuttx-driver-development/scripts/workflow-state.sh init`
...
1. **必须先读取** `~/.agents/skills/nuttx-driver-development/references/mcal_autosar_pattern.md`（使用 readFile 完整读取，包含全局约束章节）。这是强制动作，不可跳过。
...
执行：`bash ~/.agents/skills/nuttx-driver-development/scripts/workflow-state.sh complete A true`
...
**门控**: `bash ~/.agents/skills/nuttx-driver-development/scripts/workflow-state.sh gate B`
...
```bash
bash ~/.agents/skills/nuttx-driver-development/scripts/validate-requirements.sh $DRIVER_DIR/requirements.md
```

验证通过后标记 `✅ Step B 完成`。

执行：`bash ~/.agents/skills/nuttx-driver-development/scripts/workflow-state.sh complete B true`
...
**门控**: `bash ~/.agents/skills/nuttx-driver-development/scripts/workflow-state.sh gate C`
...
**⛔ 门禁：生成后必须立即执行以下命令，exit 0 才可继续，否则修复后重跑：**
```bash
bash ~/.agents/skills/nuttx-driver-development/scripts/validate-design.sh $DRIVER_DIR/design.md
```
...
执行：`bash ~/.agents/skills/nuttx-driver-development/scripts/workflow-state.sh complete C true`
...
**门控**: `bash ~/.agents/skills/nuttx-driver-development/scripts/workflow-state.sh gate D`
...
1. 如果当前会话尚未读取 `~/.agents/skills/nuttx-driver-development/SKILL.md`，则读取获取驱动通识规则（编码规范、内核 API、中断规则、同步原语等）。如果 Step A 已读取过，跳过此步。
...
**门控**: `bash ~/.agents/skills/nuttx-driver-development/scripts/workflow-state.sh gate D2`
...
>    ```bash
>    bash ~/.agents/skills/nuttx-driver-development/scripts/validate-deliverables.sh $DRIVER_DIR/design.md <项目根目录>
>    ```
...
> 5. 全部通过后执行：`bash ~/.agents/skills/nuttx-driver-development/scripts/workflow-state.sh complete D2 true`

### D.3.5 寄存器与骨架验证

**门控**: `bash ~/.agents/skills/nuttx-driver-development/scripts/workflow-state.sh gate D35`
...
执行：`bash ~/.agents/skills/nuttx-driver-development/scripts/workflow-state.sh complete D35 true`
...
执行：`bash ~/.agents/skills/nuttx-driver-development/scripts/workflow-state.sh complete D true`
...
**门控**: `bash ~/.agents/skills/nuttx-driver-development/scripts/workflow-state.sh gate E`
...
1. 读取 `~/.agents/skills/openvela-build/SKILL.md`
2. 按 skill 定义的工作流执行构建验证
3. **编译无法启动时的回退**：如果 `openvela-build` skill 的编译命令无法正常启动（如 CMake 找不到 board config、lunch target 无效等），则搜索 `~/.agents/skills/` 目录下是否有能编译该目标板的其他 skill（如 `caros-board`），找到后按该 skill 的编译流程重试
...
执行：`bash ~/.agents/skills/nuttx-driver-development/scripts/workflow-state.sh complete E true`
...
1. 读取 `~/.agents/skills/submit-pr/SKILL.md`，按其工作流执行 Fork 模式提交
